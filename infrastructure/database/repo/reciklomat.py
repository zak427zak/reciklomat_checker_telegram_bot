...
class ReciklomatRepo(BaseRepo):
    ...
    def get_all_sorted_by_status_and_occupancy(self):
        ...
        """
        return (
            self.session.query(Reciklomat)
            .order_by(Reciklomat.status.desc(), Reciklomat.occupancy.asc())
            .all()
        )
    ...
    def to_collection_reciklomats(self, page: int, per_page: int, current_user):
        ...
        for reciklomat in resources:
            is_checked = self.is_user_subscribed_to_reciklomat(
                current_user.id, reciklomat.address
            )
            checked_data = "✅" if is_checked else "⛔"
            data.append(
                {
                    "id": reciklomat.id,
                    "address": reciklomat.address,
