...
def create_manage_wishlist_keyboard(user_id):
    ...
    for item in data["items"]:
        reciklomat_button = InlineKeyboardButton(
            text=f"{item['checked_data']} {item['address']}",
            callback_data=reciklomat_callback.new(
                id=item["id"], address=item["address"], is_checked=item["is_checked"]
            ),
        )
...
def create_languages_keyboard(create_or_update):
    ...
    langs = {"ru": "🇷🇺 Русский", "en": "🇬🇧 English", "sr": "🇷🇸 Српски"}
    for key, value in langs.items():
        language_button = InlineKeyboardButton(
            text=f"{value}",
            callback_data=language_callback.new(
                id=key, create_or_update=create_or_update
            ),
        )
